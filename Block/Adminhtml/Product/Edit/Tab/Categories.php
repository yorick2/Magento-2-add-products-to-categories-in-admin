...
class Categories extends \Magento\Backend\Block\Store\Switcher
{
    /**
     * @var string
     */
    ...
    /**
     * Retrieve array of category IDs of current product
     *
     * @return array
     */
    ...
    /**
     * Returns whether product associated with category with $categoryId
     *
    ...
    {
        return in_array($categoryId, $this->getProduct()->getCategoryIds());
    }

    /**
     * Check categories block is readonly
     *
     * @return boolean
     */
    ...
    public function getChooseFromStoreHtml($storeTo)
    {
        if (!$this->_storeFromHtml) {
            ...
            foreach ($this->getCategoryCollection() as $_category) {
                if (!$this->hasCategory($_category->getId())) {
                    continue;
                }
                $optGroupLabel = $this->escapeHtml($_category->getName());
                $this->_storeFromHtml .= '<optgroup label="' . $optGroupLabel . '"></optgroup>';
                foreach ($this->getGroupCollection($_category) as $_group) {
